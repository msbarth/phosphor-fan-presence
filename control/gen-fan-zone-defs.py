            ...
            ZoneDefinition{
                ...
                %for event in zone['events']:
                    ...
                    SetSpeedEvent{
                        ...
                        %endfor
                    std::vector<SetSpeedEvent>{
                    %for pcevt in event['pc']['pcevt']:
                    SetSpeedEvent{
                        Group{
                        %for member in pcevt['group']:
                        {
                            "${member['name']}",
                            {"${member['interface']}",
                             "${member['property']}"}
                        ...
                        Timer{
                            ${pcevt['timer']['interval']}
                        },
                        std::vector<PropertyChange>{
                        %for s in pcevt['signal']:
                            PropertyChange{
...
def addPrecondition(zNum, zCond, event, events_data):
    ...
    for pce in event['precondition']['events']:
        ...
        if all('zones' in z and z['zones'] is not None and
               zNum not in z['zones'] for z in pce['zone_conditions']):
            continue
        ...
        if all('name' in z and z['name'] is not None and
               ...
               for z in pce['zone_conditions']):
            continue

        # Add set speed event group
        pcegroup = []
        pcegroups = next(g for g in events_data['groups']
                         if g['name'] == pce['group'])
        for pcemember in pcegroups['members']:
            pcemembers = {}
            pcemembers['obj_path'] = pcegroups['type']
            pcemembers['name'] = (pcegroups['type'] +
                                  pcemember)
            pcemembers['interface'] = pce['interface']
            pcemembers['property'] = pce['property']['name']
            pcegroup.append(pcemembers)
        pcevent['group'] = pcegroup

        # Add set speed actions and function parameters
        action = []
        for eActions in pce['actions']:
            actions = {}
            ...
            for p in eAction['parameters']:
                ...
                if type(eActions[p]) is not dict:
                    if p == 'property':
                        ...
                        param['type'] = str(pce['property']['type']).lower()
                    else:
                    ...
                    if p != 'map':
                        param['value'] = str(eActions[p]['value']).lower()
                    else:
        ...
        pcevent['action'] = action

        # Add property change signal handler
        pcesignal = []
        for pcepath in pcegroup:
            pcesignals = {}
            pcesignals['obj_path'] = pcepath['obj_path']
            pcesignals['path'] = pcepath['name']
            pcesignals['interface'] = pce['interface']
            pcesignals['property'] = pce['property']['name']
            pcesignals['type'] = pce['property']['type']
            pcesignal.append(pcesignals)
        pcevent['signal'] = pcesignal

        # Add optional action call timer
        pcetimer = {}
        pceinterval = "static_cast<std::chrono::seconds>"
        if ('timer' in pce) and \
           (pce['timer'] is not None):
            pcetimer['interval'] = (pceinterval +
                                    "(" + str(pce['timer']['interval']) + ")")
        else:
            pcetimer['interval'] = (pceinterval +
                                    "(" + str(0) + ")")
        pcevent['timer'] = pcetimer

        pcevents.append(pcevent)
    precond['pcevt'] = pcevents
